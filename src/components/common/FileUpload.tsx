...
export default function FileUpload({
  ...
  const handleFiles = async (files: FileList) => {
    ...
    // Filter by accepted types for drag & drop as browsers don't enforce accept there
    const matchesAccept = (file: File, accepts: string[]) => {
      ...
      return accepts.some((a) => {
        ...
        if (a.startsWith('.')) {
          return name.endsWith(a.toLowerCase())
        }
        return file.type === a
      })
    }

    const selected = fileArray.filter((f) => matchesAccept(f, acceptedTypes))
    const rejected = fileArray.length - selected.length
    if (rejected > 0) {
      toast.error(`${rejected} fil(er) afvist pga. filtype`)
    }
          ...
          {uploading ? (
            ...
            <>
              ...
              <p className="text-xs text-gray-400">
                Understøttede formater: billeder, video, lyd, PDF, Word, Excel, PowerPoint, tekst, CSV, JSON
              </p>
            </>
          )}
        </div>
